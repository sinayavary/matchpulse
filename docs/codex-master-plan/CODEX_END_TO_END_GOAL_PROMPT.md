...
7. docs/codex-master-plan/HUMAN_INPUT_REGISTRY.json
8. docs/codex-master-plan/QUALITY_GATE_MATRIX.md
9. docs/codex-master-plan/TEST_AND_RELEASE_STRATEGY.md
10. docs/codex-master-plan/RISK_SECURITY_OPERATIONS.md
11. docs/codex-master-plan/ACTIVE_PHASE.json
12. docs/codex-master-plan/PHASE_QUEUE.json
13. the exact active or selected phase pack

Authority conflicts are resolved using PROGRAM_PLAN.json authority_order, except that a stricter safety, security, migration, network, secret, or production restriction always wins. Never weaken a restriction by interpretation.

PROGRAM START
- Verify that PROGRAM_PLAN.json exists and schema-validates.
...
- Verify repository remote, main ancestry, clean/collision-safe working state, branch protection assumptions, and current phase state.
...
2. Read program plan, phase state, human inputs, gates, dependencies, locks, risk class, autonomy level, and prior evidence.
3. Select only the highest-priority eligible phase declared by PROGRAM_PLAN.json.
4. Never invent, skip, reorder, self-approve, or silently broaden a phase.
...
9. Never add or expose betting recommendations, wager/stake/payout/profit/expected-value/edge/trade execution, gambling UI, or ordinary-user wallet requirements.
10. Never expose raw provider payloads, provider identities, assigned weights, model coefficients, private thresholds, feature hashes, internal specialist contributions, proof blobs, credentials, stack traces, debug lineage, or private policy values in public surfaces.
...
18. Update phase and program evidence atomically with exact files, SHAs, commands, counts, durations, migration/network/public-contract/security status, rollback reference, and known limitations.
...
21. After publication, mark only the completed phase complete, activate only a declared successor whose dependencies and gates are satisfied, then continue automatically.
...
HUMAN INPUTS
- Read HUMAN_INPUT_REGISTRY.json.
...
- Never request secret values in chat or Git. Request secret-store/environment location and variable names only.
...
PROGRAM COMPLETION
Report PROGRAM_COMPLETE only when every required PROGRAM_PLAN.json phase is completed, all hard gates are approved, release gates pass, immutable release references exist, staging/production or approved demo smoke tests pass, rollback evidence exists, and the final program report contains exact commits, migrations, artifacts, environments, tests, scans, limitations and operational references.
...
No mode may auto-approve migrations, real service access, production mutation, secrets, or final release.
