...
This phase only adds a controlled CLI worker foundation.
...
```powershell
pnpm.cmd --filter @matchpulse/worker dev -- --fixtureId 17952170 --competitionId 430 --startEpochDay 20608 --dry-run
```
...
Only use it after a dry-run looks correct:

```powershell
pnpm.cmd --filter @matchpulse/worker dev -- --fixtureId 17952170 --competitionId 430 --startEpochDay 20608 --execute
```

Execute mode is explicit opt-in only.
