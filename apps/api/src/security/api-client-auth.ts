...
export type AuthenticatedClient = { hash: string; applicationId: string; scopes: readonly string[]; expiresAt: number; revokedAt?: number };
export type ApiClientAuth = {
  createCredential(applicationId: string, scopes: ExternalScope[]): MaybePromise<{ clientId: string; clientSecret: string; scopes: ExternalScope[] } | undefined>;
  issueToken(clientId: string, clientSecret: string, scopeText?: unknown): MaybePromise<{ access_token: string; token_type: string; expires_in: number; scope: string } | undefined>;
  authenticate(token: string): MaybePromise<AuthenticatedClient | undefined>;
  findCredential?(clientId: string): MaybePromise<Pick<Credential, "id" | "clientId" | "applicationId"> | undefined>;
  revokeCredential(clientId: string): MaybePromise<void>;
  revokeApplication(applicationId: string): MaybePromise<void>;
  revokeToken(token: string): MaybePromise<void>;
