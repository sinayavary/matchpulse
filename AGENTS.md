...
The exact identity `MATCHES-RAILWAY-ENVIRONMENT-AUTH-A` / `MATCHES-RAILWAY-ENVIRONMENT-AUTH-A-v1`, baseline `f491c805c659d860e41351bb231f606603743132`, may use only the Railway Public GraphQL API at `https://backboard.railway.com/graphql/v2` with `RAILWAY_API_TOKEN` supplied solely from the secure process environment through `Authorization: Bearer <secure environment value>`. The credential value must never be printed, copied, persisted, hashed, truncated, or logged; `RAILWAY_TOKEN`, Railway CLI, and implicit project binding are forbidden. A separately approved first gate permits only one read-only authorization/project preflight. Environment creation requires its own separate human gate. No permission is inferred from `state=ready` or `human_approved=true`; service, variable, domain, database, deployment, migration, backfill, source-connection, and cron operations remain forbidden.

### Matches Railway transport diagnostic governance exception

The exact identity `MATCHES-RAILWAY-TRANSPORT-DIAGNOSTIC-A` / `MATCHES-RAILWAY-TRANSPORT-DIAGNOSTIC-A-v1`, baseline `13a97eaeb382f7465838a7c627d7454ab505181a`, is a narrowly scoped transport-diagnostic and preflight-retry governance exception. Governance publication authorizes no network access and the phase must begin `awaiting_human_approval` with `human_approved=false`.

After separate human activation, Gate 1 alone may perform at most one DNS lookup, one TCP 443 check and one HTTPS reachability request to `https://backboard.railway.com/graphql/v2`, with no token, no Authorization header and no GraphQL/account/project/environment data. Gate 1 permits no retry and records only one safe transport classification. Gate 2 requires separate human approval and `HTTP_ENDPOINT_REACHABLE`; it may perform only the bounded read-only project preflight retry. Raw bodies, headers, errors, IPs, certificate details and proxy values are forbidden. Mutations, staging creation, services, variables, domains, databases, deployments, migrations, backfills, production HTTP and Railway CLI remain forbidden.
